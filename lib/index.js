...
CodeMirror.defineOption('autocomplete', false, function(cm, value) {
  ...
  CodeMirror.defineExtension('addKeywordsFromString', function(string) {
    const ac = this.state.autocomplete;
    const words = [];
    Autocomplete.textToWords(string, ac.isWordChar, word => {
      if (ac.userKeywords.indexOf(word) === -1) {
        if (word.length && (word[0] < '0' || word[0] > '9')) {
          if (ac.keywords.indexOf(word) === -1) {
            words.push(word);
          }
        }
      }
    });
    ac.cm.setOption('keywords', ac.userKeywords.concat(words));
    ac.reset();
  });
  CodeMirror.defineExtension('setKeywords', userKeywords => {
    this.setOption('keywords', userKeywords);
    this.state.autocomplete.reset();
  });
});
...
class Autocomplete {
  ...
  reset() {
    ...
    Autocomplete.textToWords(cm.getValue(), this.isWordChar, word => {
      if (word.length && (word[0] < '0' || word[0] > '9')) {
    ...
    this.userKeywords = cm.getOption('keywords') || [];
    this.userKeywords.forEach(word => this.dictionary.addWord(word));
  }

  static textToWords(text, isWordChar, wordCallback) {
    let startWord = -1;
  ...
  beforeChange = (cm, change) => {
    ...
    Autocomplete.textToWords(lines, this.isWordChar, word => {
  ...
  showHint(from, to) {
    ...
    // console.log('word: %s', word, matches);
